...
def get_exchange_rate_by_as_of(*, code: str, as_of: str, column: str, currency: str) -> str:
    sql_template = """
    CASE
      ...
      ELSE COALESCE(
        (
          ...
          WHERE er.base_code = ${currency_alias}.currency
            ...
            AND er.rate_date <= ${as_of_alias}.as_of::date
        ...
        (
          ...
          WHERE er2.base_code = ${target_code}
            ...
            AND er2.rate_date <= ${as_of_alias}.as_of::date
