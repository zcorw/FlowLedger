...
- 基础设施
  - `docker-compose.yml`：PostgreSQL + API 服务（API 从 `api/Dockerfile` 构建）
- API 代码与镜像
...
2) 目录结构确认
   - 确保上述“文件总览”中的路径存在；`docker-compose.yml` 将从 `api/Dockerfile` 构建镜像
...
- 8000 端口占用：修改端口映射（编辑 `docker-compose.yml` 的 `ports: 8000:8000`），或释放端口
...
- 根据需要扩展 `docker-compose.yml`（健康检查、资源限制）
