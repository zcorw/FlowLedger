...
CREATE OR REPLACE FUNCTION expense.fn_seed_expense_categories_for_new_user()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  INSERT INTO expense.expense_categories (user_id, name, tax)
  SELECT NEW.id, c.name, c.tax
  FROM (
    VALUES
      ('餐饮', 0.08),         -- 餐食、买菜、咖啡、零食
      ('住房', 0.1),         -- 房租、房贷、居住相关费用
      ('交通', 0.1),         -- 公交地铁、打车、油费、停车
      ('日用品', 0.1),         -- 零售消费、日用品
      ('健康', 0.1),         -- 医疗、药品、健身
      ('教育', 0.1),         -- 课程、书籍、培训
      ('旅行', 0.1),         -- 旅途交通、住宿、门票
...
CREATE TABLE IF NOT EXISTS expense.expense_categories (
  ...
  tax        NUMERIC(7,6) NOT NULL DEFAULT 0.0,
...
DROP TRIGGER IF EXISTS trg_seed_expense_categories ON "user".users;
CREATE TRIGGER trg_seed_expense_categories
AFTER INSERT ON "user".users
FOR EACH ROW EXECUTE FUNCTION expense.fn_seed_expense_categories_for_new_user();
